...
func GetProviderForModel(modelName string, apiKeys map[string]string) (Provider, error) {
	...
	if containsAny(modelLower, []string{"claude", "haiku", "sonnet", "opus"}) {
		if apiKeys["anthropic"] != "" {
			provider, _ := NewProvider(ProviderAnthropic, apiKeys["anthropic"])
			return provider, nil
		}
		// No API key - fall through to try other providers
	}

	// OpenAI models
	if containsAny(modelLower, []string{"gpt-3.5", "gpt-4", "o1"}) {
		if apiKeys["openai"] != "" {
			provider, _ := NewProvider(ProviderOpenAI, apiKeys["openai"])
			return provider, nil
		}
		// No API key - fall through to try other providers
	}
	...
	if containsAny(modelLower, []string{"gemini", "palm"}) {
		if apiKeys["google"] != "" {
			provider, _ := NewProvider(ProviderGoogle, apiKeys["google"])
			return provider, nil
		}
		// No API key - fall through to try other providers
	}
	...
	if containsAny(modelLower, []string{"local", "phi", "llama", "mistral"}) {
		provider, _ := NewProvider(ProviderLocal, "")
		return provider, nil
	}

	// No API keys configured - default to local provider
