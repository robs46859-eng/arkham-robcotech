...
async def create_checkout_session(request: CheckoutRequest):
    ...
    pricing = settings.pricing.get(request.plan)
    if not pricing:
        raise HTTPException(status_code=400, detail="Invalid founder package")

    if request.commitment not in {"monthly", "quarterly"}:
    ...
    customer = await stripe_service.create_customer(
        ...
        metadata={"company_name": request.company_name, "plan": request.plan},
    )
    customer_id = customer["id"]

    base_amount = Decimal(str(pricing["monthly"]))
    if request.commitment == "quarterly":
        amount = (base_amount * Decimal("0.65") * Decimal("3")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    else:
...
async def list_plans():
    ...
    return {"plans": settings.pricing}


@app.get("/api/v1/plans/{plan_id}")
async def get_plan(plan_id: str):
    """Get plan details"""
    plan = settings.pricing.get(plan_id)
    if not plan:
...
async def _handle_stripe_webhook(request: Any):
...
async def stripe_webhook_legacy(request: Any):
    """Legacy Stripe webhook endpoint kept for dashboard compatibility during cutover."""
    return await _handle_stripe_webhook(request)
