...
from app.tasks import create_executor_registry
...
async def lifespan(app: FastAPI):
    ...
    app.state.executor_registry = create_executor_registry()

    await app.state.checkpoint_store.initialize()
    await app.state.task_queue.initialize()

    # Register built-in flows
    await app.state.flow_registry.register_built_in_flows()

    # Initialize worker
    app.state.worker = WorkflowWorker(
    ...
    logger.info("Worker initialized (poll_interval=1.0s)")
...
async def worker_run_once():
    ...
    Useful for testing and manual workflow stepping.
    Returns the task result or None if no tasks available.
    """
    if not hasattr(app.state, "worker"):
        return {"error": "Worker not initialized"}

    result = await app.state.worker.process_next_task()
    if result is None:
        return {"status": "no_tasks", "message": "No pending tasks in queue"}
...
async def worker_drain(max_tasks: int = 100):
    """
    Process up to max_tasks from the queue

    Useful for batch processing or clearing the queue.
    Returns summary of processed tasks.
    """
    if not hasattr(app.state, "worker"):
        return {"error": "Worker not initialized"}

    result = await app.state.worker.drain(max_tasks=max_tasks)
    return result
...
async def worker_status():
    ...
    Returns queue depths and worker state.
    """
    if not hasattr(app.state, "worker"):
        return {"error": "Worker not initialized"}

    return {
        "running": app.state.worker._running,
        "poll_interval": app.state.worker.poll_interval,
        "pending_tasks": await app.state.task_queue.pending_count if hasattr(app.state.task_queue, "pending_count") else "unknown",
    }
