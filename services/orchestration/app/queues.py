...
except ModuleNotFoundError:  # pragma: no cover - local unit tests may not install redis
    redis = None
try:
    from orjson import dumps, loads
except ModuleNotFoundError:  # pragma: no cover - local unit tests may not install orjson
    import json
...
DEFAULT_QUEUE_NAME = "workflow_tasks"


def _serialize(value: Dict[str, Any]) -> str:
    """Normalize JSON serialization across json/orjson backends."""
    payload = dumps(value)
...
class TaskQueue:
    ...
    async def initialize(self):
        ...
        if redis is None:
            raise RuntimeError("redis is required to initialize TaskQueue")
    ...
    async def enqueue(
        ...
        task = {
            ...
            "step_name": step_name,
            "step_data": _serialize(step_data),
            "priority": priority,
        ...
        if delay_seconds > 0:
            ...
            await self.client.zadd(
                ...
                {_serialize(task): int(datetime.utcnow().timestamp()) + delay_seconds}
            )
        else:
            # Add to main queue (using sorted set for priority)
            await self.client.zadd(
                queue_name,
                {_serialize(task): -priority}  # Negative for descending order
            )
    ...
    async def complete_task(
        ...
        """Compatibility wrapper for the worker-facing completion call."""
        del output
        await self.complete(queue_name=queue_name, task_id=task_id)
